...
| **Exit codes** | `0` on success, including a reader closing stdout (`\| head`). `1` is an answer, not a failure: `--check-budgets` exits `1` when any budget has reached its `warn` threshold. `2` is the tool failing — a flag it does not know, a config or source it could not read, a write that did not happen, `--check-update` unable to ask or to cache. The exception is `--claude-code-hook`, which exits `1` when it fails, because Claude Code reads a hook's `2` as a message for the model. Other codes are not used, and may be given a meaning in a minor release. |
...
| **JSON output** | `--summary-json`, `--json`, `--routing-json` and `--check-budgets` each print one object carrying `"schema_version": 1`, and `--schema` prints the glossary of every key and enum value in them — a test fails if a document carries anything the glossary lacks. Within a version keys are only added, never removed, renamed, or given a different meaning or type. Ignore keys you do not know. An absent value is `null`, not `0` — `cost` especially — and that is part of the meaning. |
| **CSV output** | `--csv` and `--routing-csv` columns are appended, never inserted, reordered or removed, so a consumer reading by position keeps working. A text field that a spreadsheet would run as a formula -- one beginning `=`, `+`, `-` or `@`, or a tab or carriage return -- is written with a leading apostrophe; a number never is. Read names from `--json` if you need them byte for byte. |
| **Journal** | `usage.db` stores its schema version in `PRAGMA user_version` (currently `1`). A release can read every journal an earlier release wrote. A writer refuses a journal stamped with a *newer* version than it knows rather than writing into it. Rows are deleted only by `--prune-journal`, on the command, and never by anything else. See [`data-model.md`](data-model.md). |
