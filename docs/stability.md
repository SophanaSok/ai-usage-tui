...
| **JSON output** | `--summary-json`, `--json`, `--routing-json` and `--check-budgets` each print one object carrying `"schema_version": 1`. Within a version keys are only added, never removed, renamed, or given a different meaning or type. Ignore keys you do not know. An absent value is `null`, not `0` — `cost` especially — and that is part of the meaning. |
