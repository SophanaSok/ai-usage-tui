...
- **The footer reads the bindings table, and fits itself by measuring.** `ui::keys` was written
  to end the hand-written copies of the key bindings, and four of five went; the footer kept
  its own, because it abbreviates (`1-4`, `j/k`) and folds the panel keys into one run below a
  width. That width was `120` — the full line's width on the day it was written, and a fact
  ...
  is ever cut off; another pins that the form changes exactly one column short of fitting.
