...
- Tests are hermetic and must stay that way: anything exercising `load_usage` or `print_once` needs an explicit `--claude-dir`, or it reads the developer's real `~/.claude/projects` — and, for the billing decision, their real `~/.claude.json`. The config document is derived from `claude_dir` (`<claude_dir>/../../.claude.json`), so a fixture root resolves to a file that does not exist; pass `claude_json` when a test needs a planted one.
